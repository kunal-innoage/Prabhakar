...
class EverstoxShopOrder(models.Model):
    ...
    def prepare_order_and_order_data(self):
        ...
        # Filter Orders Without Attachment (Wayfair and Amazon only) 
        way_ama_no_label_orders = []
        for order in self:
            if order.sale_order_id.wayfair_order_id or order.sale_order_id.amazon_order_id:
                attachments = self.env['ir.attachment'].search([('res_model','=','sale.order'),('res_id','=',order.sale_order_id.id)])
                if len(attachments) == 0:
                    way_ama_no_label_orders += [order]
        for order in self:
            if order not in way_ama_no_label_orders:
                ...
                _logger.info("..........  Labels not found for  -   ..%r..........", order.sale_order_id.name )
