...
class Seller(models.Model):
    ...
    def create_sale_order(self,order):
        ...
        if not sale_order:
            ...
            logger.info("Before change date >>>>>>" +str(sale_order.date_order))

            logger.info(str(sale_order.date_order) + ">>>>>>>>>>>" + s2)
            ...
            for picking in sale_order.picking_ids:
                picking.cdiscount_order_id = sale_order.cdiscount_order_id
            shipping_id = self._create_shipping_customer(lines[0].get("shippingAddress"), customer_id)
            sale_order.write({'partner_id':customer_id, 'partner_invoice_id':billing_id, 'partner_shipping_id' : shipping_id, 'state': odoo_state, 'mirakl_order_state' : mirakl_state, 'cdiscount_shop_id': self.id,'market_place_shop': self.name})
