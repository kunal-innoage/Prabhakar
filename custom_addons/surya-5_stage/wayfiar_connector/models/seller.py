...
class Seller(models.Model):
    ...
    def get_wayfair_shop(self,orderNumber):
        ...
        shops = self.env['wayfair.seller'].search([])
        way_de_shop = way_uk_shop = None
        for shop in shops:
            if shop.warehouse_id.code == 'ETL':
                way_de_shop = shop
            else:
                way_uk_shop = shop
        shop_id = way_de_shop.id if orderNumber.startswith("DE") else way_uk_shop.id
        return shop_id
    ...
    def action_update_marketplace_shop_name(self):
        ...
        for order in wayfair_orders:
            if order.wayfair_shop_id:
                order.market_place_shop = order.wayfair_shop_id.name
