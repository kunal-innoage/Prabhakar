        ...
        for res in self:
            if res.order_id:
                ...
                if res.label and so:
                    ...
                    attachments = self.env['ir.attachment'].search([('res_model','=','split.labels'),('res_id','=',res.id)])
                    exisisting_attachments  = self.env['ir.attachment'].search([('res_model','=','sale.order'),('res_id','=',so.id)])
                    if not len(exisisting_attachments) > 0:
                        for attachment in attachments:
                            self.attach_label_to_order(so, attachment)
                    else:
                        logger.info("Label already mapped to sale order %s ",so )
