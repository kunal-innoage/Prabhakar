...
class WarehouseProcessedOrders(models.Model):
    ...
    blade_order_id = fields.Many2one("blade.shop.order", string="Blade Order")

    def unmap_orders(self):
        for order in self:
            order.blade_mapped_state = "unmapped"

    def create_blade_mapping_order_line_data(self, order, sale_order,blade_order):
    ...
    def map_order_with_everstox(self):
        ...
        for order in self:
            if order.warehouse == 'BOH' and order.blade_mapped_state == "unmapped":
                if order.sale_order_id and not order.sale_order_id.blade_order_mapped:
                    ...
                    if billing_address_id and shipping_address_id:
                        ...
                        if '-1' in order.order_id:
                            order_id = order.order_id.split('-')[0]
                        ...
                        order.sale_order_id.blade_order_mapped = True
                        order_line_ids = self.create_blade_mapping_order_line_data(order, order.sale_order_id,new_order_id)
                    ...
                    _logger.info("This sale order already mapped with blade. This might be the case for multilne order")
