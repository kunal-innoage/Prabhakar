...
class BladeShopOrder(models.Model):
    ...
    label_sent = fields.Boolean(string='Label Sent')
    ...
    label_status = fields.Selection(string='Label Stauts', selection=[('sent', 'Label Sent'), ('not', 'Label Not Sent'),],default="not")
    ...
    def prepare_order_and_order_data(self):
        ...
        for order in self:
            ...
            if order.sale_order_id.wayfair_order_id:
                ...
                if order.sale_order_id.wayfair_carrier_code == 'UKML':
                    shipping_code = "DHL_PrePaid"
                elif order.sale_order_id.wayfair_carrier_code == 'XDPE':
                    shipping_code = "XDP_Prepaid"
                ...
                #Changes requested by blade team
            ...
            order_data = {
                ...
                "shipping_address": {
                    ...
                    "address_one": order.shipping_address_1,
                    "address_two": order.shipping_address_2 or "",
                    # "address_three": order.shipping_addres,
                    "town": order.shipping_city,
                    "company": "Surya",
                    "country_id": order.shipping_country_code or "GB",
                    "postcode": order.shipping_zip,
                    "telephone": order.shipping_phone or "",
                },
                "lines": order_lines,
                "shipping": {
            ...
            self.send_data_to_blade(order_data,order)

    def send_data_to_blade(self,data,order):
        logger.info(data)
        ...
        if response:
            if response.get('data'):
                ...
                order.write({'blade_platform_id': response.get('data').get('id'),'blade_mapped_state':'mapped'})
            else:
    ...
    def send_labels_to_blade(self):
        for order in self:
            ...
            for attachment in attachments:
                if order.blade_platform_id:
                    self.send_label_to_api(attachment,order)
                else:
                    logger.info("Blade ID is not assigned to this order %s", str(order.order_number))
        ...
        if token:
            ...
            for label_place in ['shipping_label', 'document']:
                ...
                try:
                    ...
                    logger.info(err)
                if response:
                    if response.get('data') and response.get('data').get('success') and response.get('data').get('success') == True:
                        order.label_status = 'sent'
                    logger.info(response)
        else:
...
class WarehouseSaleOrderLine(models.Model):
    ...
    _description = 'Blade Shop Order Line'
