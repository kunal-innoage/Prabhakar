...
class WayfairB2bShop(models.Model):
    ...
    starting_number = fields.Char(string='Starting Number')
    ...
    def get_orders(self,order=False):
        ...
        api_config = self.env['wayfair.b2b.orders'].sudo().search([],limit=1)
        from_date = self.date_created_start or (datetime.datetime.now().date() - datetime.timedelta(days=3))
        wayfair_shop = self.env['wayfair.b2b.shop'].search([],limit=1)
        API_URL = False
        ...
        if not api_config:
            ...
            if auth_token:
                headers = {
                    ...
                    'authorization': 'Bearer ' +  wayfair_shop.auth_token      }
    ...
    def get_new_token(self):
        api_config = self.env['wayfair.b2b.orders'].sudo().search([], limit=1).wayfair_api_id
        if api_config:
            ...
            if api_config:
                ...
                if response:
                    wayfair_shop = self.env['wayfair.b2b.shop'].search([],limit=1)
                    wayfair_shop.auth_token = response.get("access_token")
    ...
    def create_sale_order(self,order):
        ...
        sale_order = self.env['sale.order'].search(['|',('wayfair_b2b_order_id.purchase_order','=',order.get('poNumber')),('wayfair_b2b_order_number', '=', order.get('poNumber'))], limit=1) or False
        ...
        if not sale_order:
            ...
            billing_id = self._create_billing_customer(order, customer_id)
            shipping_id = self._create_shipping_customer(order.get("shipTo"), customer_id,order)
            sale_order.write({'wayfair_b2b_order_number':order.get('poNumber'),'wayfair_b2b_shop_id':wayfair_shop.id,'partner_id': customer_id, 'date_order': order_date,  'partner_invoice_id': billing_id, 'partner_shipping_id': shipping_id,'market_place_shop': wayfair_shop.name,'warehouse_id': warehouse.id})
            self.env.cr.commit()
            updated_order += 1
        return sale_order 

    def action_update_marketplace_shop_name(self):
        wayfair_orders = self.env['sale.order'].search(['|',('wayfair_b2b_order_id.purchase_order','!=',False),('wayfair_b2b_order_number','!=',False)])
        for order in wayfair_orders:
            order_number = order.wayfair_b2b_order_number if order.wayfair_b2b_order_number else order.wayfair_b2b_order_id.purchase_order
            shop = self.get_wayfair_shop(order_number)
            logger.info("Shop Changed from %s to %s", order.wayfair_b2b_shop_id.name, shop.name )
            order.wayfair_b2b_shop_id = shop.id
            order.market_place_shop = shop.name
            if order.wayfair_b2b_order_id.purchase_order and not order.wayfair_b2b_order_number:
                order.wayfair_b2b_order_number = order.wayfair_b2b_order_id.purchase_order
    ...
    def delete_order_force_wayfair(self):
        orders = self.env['sale.order'].search(['|',('wayfair_b2b_order_id.purchase_order','!=',False),('wayfair_b2b_order_number','!=',False)])
        print(len(orders))
        duplicate_orders = []
        total_orders = []
        for order in orders:
            if order.wayfair_b2b_order_number or order.wayfair_b2b_order_id:
                order_number = order.wayfair_b2b_order_number or order.wayfair_b2b_order_id.purchase_order
                if order_number in total_orders:
                    duplicate_orders.append(order.id)
                else:
                    total_orders.append(order_number)
                    pass
        print(len(duplicate_orders))
        deleted_orders = 0
        to_delete_orders = self.env['sale.order'].browse(duplicate_orders)
        for order in to_delete_orders:
            if deleted_orders <= 500:
                order_name = order.name
                ab = self.env.cr.execute("""DELETE from sale_order WHERE id=%s""" %(order.id))
                ab = self.env.cr.execute("""DELETE from stock_picking WHERE origin='%s'""" %(order_name))
                logger.info("Order %s deleted with it's deliveries", order.name)
