...
class SaleOrder(models.Model):
    ...
    is_dropship_order = fields.Boolean("Is a dropship order")
    ...
    def _create_billing_customer(self, order, billing_address, customer_id):
        full_name = False
        if billing_address.get('firstname'):
            ...
            if billing_address.get('lastname'):
                full_name = billing_address.get('lastname')
        billing_customer = self.env['res.partner'].search([('type', '=', 'invoice'), ('parent_id', '=', customer_id),('name', 'ilike', full_name)], limit=1)
        if not billing_customer:
            ...
            if len(country) <= 0:
                ...
                if not country:
                    country = self.env['res.country'].search(['|',('name', '=',  order.get('channel').get('label').lower()), ('code', '=', order.get('channel').get('code'))])
    ...
    def _create_shipping_customer(self, order, shipping_address, customer_id):
        ...
        if shipping_address.get('firstname'):
            ...
            if shipping_address.get('lastname'):
                full_name = shipping_address.get('lastname')
        shipping_customer = self.env['res.partner'].search([('type', '=', 'delivery'), ('parent_id', '=', customer_id),('name', 'ilike', full_name)], limit=1)
    ...
    def check_for_customer_updates(self, order, sale_order):
        ...
        if billing_data and shipping_data:
            ...
            if len(sale_order.partner_id.country_id) == 0:
                ...
                if len(sale_order.partner_id.country_id) == 0:
                    try:
                        if order.get("shipping_zone_code"):
                            sale_order.partner_id.country_id = self.env['res.country'].search([('name', '=', order.get('shipping_zone_label'))])
                        if len(shipping_address.country_id) == 0:
                            shipping_address.country_id = sale_order.partner_id.country_id
                    except:
                ...
                if len(sale_order.partner_id.country_id) == 0:
                    try:
                        if order.get("channel"):
                            sale_order.partner_id.country_id = self.env['res.country'].search([('code', '=', order.get('channel').get('code'))])
                            if len(shipping_address.country_id) == 0:
                                shipping_address.country_id = sale_order.partner_id.country_id
                    except:
                        _logger.info("!!!!!!!!! Country not found, leaving empty  order id - %r ;;;;;;", sale_order.id)
        ...
        if not sale_order:
            ...
            if customer_id:
                if order.get('customer').get("billing_address") !=None and order.get('customer').get("shipping_address") != None:
                    try:
                        billing_id = self._create_billing_customer(order, order.get('customer').get("billing_address"), customer_id)
                    except:
                        _logger.info("Billing creation error~~~~~~~%r ;;;;;",order.get('customer').get("billing_address"))
                        billing_id = False
                    try:
                        shipping_id = self._create_shipping_customer(order, order.get('customer').get("shipping_address"), customer_id)
                    except:
            ...
            if customer_id and billing_id and shipping_id and warehouse_id and order_line:
                ...
                if not partner.country_id:
                    if order.get("shipping_zone_code"):
                        country_id = self.env['res.country'].search([('name', '=', order.get('shipping_zone_label'))])
    ...
    def send_api_calls(self, shop, ded_qty):
        ...
        if offer_id:
            ...
            if shop.shop_id.shop_id:
                call += "?shop_id=" + shop.shop_id.shop_id
            try:
...
class SaleOrderLine(models.Model):
    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        for line in self:
            if line.order_id.is_dropship_order:
